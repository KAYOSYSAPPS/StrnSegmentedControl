...
#import "StrnSegmentedControl.h"
#import <QuartzCore/QuartzCore.h>
#import <math.h>

@interface StrnScrollView : UIScrollView
@end

@interface StrnSegmentedControl ()
...
@property (nonatomic, strong) StrnScrollView *scrollView;

@end

@implementation StrnScrollView
...
@implementation StrnSegmentedControl
...
- (id)initWithSectionTitles:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.type = StrnSegmentedControlTypeText;
    }
...
- (id)initWithSectionImages:(NSArray*)sectionImages sectionSelectedImages:(NSArray*)sectionSelectedImages {
    ...
    if (self) {
        ...
        self.type = StrnSegmentedControlTypeImages;
    }
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.sectionSelectedImages = sectionSelectedImages;
		self.sectionTitles = sectiontitles;
        self.type = StrnSegmentedControlTypeTextImages;
    }
...
- (void)commonInit {
    self.scrollView = [[StrnScrollView alloc] init];
    ...
    self.selectionStyle = StrnSegmentedControlSelectionStyleTextWidthStripe;
    self.selectionIndicatorLocation = StrnSegmentedControlSelectionIndicatorLocationUp;
    self.segmentWidthStyle = StrnSegmentedControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(StrnSegmentedControlSelectionIndicatorLocation)selectionIndicatorLocation {
	_selectionIndicatorLocation = selectionIndicatorLocation;
	
	if (selectionIndicatorLocation == StrnSegmentedControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(StrnSegmentedControlSegmentWidthStyle)segmentWidthStyle {
    // Force StrnSegmentedControlSegmentWidthStyleFixed when type is StrnSegmentedControlTypeImages.
    if (self.type == StrnSegmentedControlTypeImages) {
        _segmentWidthStyle = StrnSegmentedControlSegmentWidthStyleFixed;
    } else {
        _segmentWidthStyle = segmentWidthStyle;
    }
}

- (void)setBorderType:(StrnSegmentedControlBorderType)borderType {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == StrnSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            BOOL locationUp = (self.selectionIndicatorLocation == StrnSegmentedControlSelectionIndicatorLocationUp);
            BOOL selectionStyleNotBox = (self.selectionStyle != StrnSegmentedControlSelectionStyleBox);
            ...
            CGRect rect;
            if (self.segmentWidthStyle == StrnSegmentedControlSegmentWidthStyleFixed) {
...
//                rectDiv = CGRectMake((self.segmentWidth * idx) - (self.verticalDividerWidth / 2), self.selectionIndicatorHeight * 2, self.verticalDividerWidth, self.frame.size.height - (self.selectionIndicatorHeight * 4));
            ...
            } else if (self.segmentWidthStyle == StrnSegmentedControlSegmentWidthStyleDynamic) {
...
//                verticalDividerLayer.frame = rectDiv;
    ...
    } else if (self.type == StrnSegmentedControlTypeImages) {
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == StrnSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
    ...
    } else if (self.type == StrnSegmentedControlTypeTextImages){
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
    ...
    if (self.selectedSegmentIndex != StrnSegmentedControlNoSegment) {
        if (self.selectionStyle == StrnSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                ...
                if (self.selectionStyle == StrnSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    // Border layer
    if (self.borderType & StrnSegmentedControlBorderTypeTop) {
...
- (void)setArrowFrame {
    ...
    if (self.selectionIndicatorLocation == StrnSegmentedControlSelectionIndicatorLocationDown) {
    ...
    if (self.selectionIndicatorLocation == StrnSegmentedControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == StrnSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == StrnSegmentedControlSelectionStyleTextWidthStripe &&
            sectionWidth <= self.segmentWidth &&
            self.segmentWidthStyle != StrnSegmentedControlSegmentWidthStyleDynamic) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
            ...
            if (self.segmentWidthStyle == StrnSegmentedControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
    ...
    if (self.type == StrnSegmentedControlTypeText && self.segmentWidthStyle == StrnSegmentedControlSegmentWidthStyleFixed) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == StrnSegmentedControlTypeText && self.segmentWidthStyle == StrnSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == StrnSegmentedControlTypeTextImages && self.segmentWidthStyle == StrnSegmentedControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == StrnSegmentedControlTypeTextImages && StrnSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        } else if (self.type == StrnSegmentedControlTypeTextImages || self.type == StrnSegmentedControlTypeText) {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if (index == StrnSegmentedControlNoSegment) {
        ...
        if (animated) {
            ...
            if(self.selectionStyle == StrnSegmentedControlSelectionStyleArrow) {
                ...
                if ([self.selectionIndicatorStripLayer superlayer] == nil) {
                    ...
                    if (self.selectionStyle == StrnSegmentedControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
                        [self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
